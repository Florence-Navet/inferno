...
TEST(ProtocolParserRegister,
  ...
  const OsInfoPayload expected = FrameBuilder::makeOsInfoPayload();
  const std::vector<std::uint8_t> input = FrameBuilder::makeRawOsInfoPayload();

  const OsInfoPayload result = ProtocolParser::parseOsInfoPayload(input);
...
TEST(ProtocolParserRegister, should_accept_max_hostname_length) {
  ...
  const OsInfoPayload expected =
      FrameBuilder::makeOsInfoPayload(OSType::MAC, ArchType::ARM, longHostname);

  const std::vector<std::uint8_t> input = FrameBuilder::makeRawOsInfoPayload(
      OSType::MAC, ArchType::ARM, longHostname);
...
TEST(ProtocolParserRegister, should_decode_hostname_length_as_big_endian) {
  ...
  const OsInfoPayload expected = FrameBuilder::makeOsInfoPayload(
      OSType::WINDOWS, ArchType::X86, longHostname);
...
TEST(ProtocolParserRegister,
  ...
  EXPECT_THROW(ProtocolParser::parseOsInfoPayload(input), InvalidSize);
}

TEST(ProtocolParserRegister, should_reject_empty_hostname) {
  const std::vector<std::uint8_t> input = FrameBuilder::makeRawFailingOsInfoPayload(
      static_cast<std::uint8_t>(OSType::WINDOWS),
      static_cast<std::uint8_t>(ArchType::X86), 0x00,
      Protocol::TEST_OS_VERSION_LEN, Protocol::TEST_CURRENT_USER_LEN, {});
...
TEST(ProtocolParserRegister, should_reject_empty_os_version) {
  const std::vector<std::uint8_t> input = FrameBuilder::makeRawFailingOsInfoPayload(
      static_cast<std::uint8_t>(OSType::LINUX),
      static_cast<std::uint8_t>(ArchType::X64), Protocol::TEST_HOSTNAME_LEN, 0,
      Protocol::TEST_CURRENT_USER_LEN, Protocol::TEST_HOSTNAME, {},
...
TEST(ProtocolParserRegister, should_reject_empty_current_user) {
  const std::vector<std::uint8_t> input = FrameBuilder::makeRawFailingOsInfoPayload(
      static_cast<std::uint8_t>(OSType::LINUX),
      static_cast<std::uint8_t>(ArchType::X64), Protocol::TEST_HOSTNAME_LEN,
      Protocol::TEST_OS_VERSION_LEN, 0, Protocol::TEST_HOSTNAME,
...
TEST(ProtocolParserRegister,
  ...
  const std::vector<std::uint8_t> input = FrameBuilder::makeRawFailingOsInfoPayload(
      static_cast<std::uint8_t>(OSType::LINUX),
      static_cast<std::uint8_t>(ArchType::X64), 10,
      Protocol::TEST_OS_VERSION_LEN, Protocol::TEST_CURRENT_USER_LEN,
      Common::bytesFromString("abc"),  // only 3 bytes
      Protocol::TEST_OS_VERSION, Protocol::TEST_CURRENT_USER);
...
TEST(ProtocolParserRegister, should_reject_unknown_os_type) {
  const std::vector<std::uint8_t> input = FrameBuilder::makeRawFailingOsInfoPayload(
      Common::INVALID_ENUM_VALUE, static_cast<std::uint8_t>(ArchType::X64),
      Protocol::TEST_HOSTNAME_LEN, Protocol::TEST_OS_VERSION_LEN,
      Protocol::TEST_CURRENT_USER_LEN, Protocol::TEST_HOSTNAME,
      Protocol::TEST_OS_VERSION, Protocol::TEST_CURRENT_USER);

  EXPECT_THROW(ProtocolParser::parseOsInfoPayload(input), InvalidFieldValue);
}

TEST(ProtocolParserRegister, should_reject_unknown_arch) {
  const std::vector<std::uint8_t> input = FrameBuilder::makeRawFailingOsInfoPayload(
      static_cast<std::uint8_t>(OSType::LINUX), Common::INVALID_ENUM_VALUE,
      Protocol::TEST_HOSTNAME_LEN, Protocol::TEST_OS_VERSION_LEN,
      Protocol::TEST_CURRENT_USER_LEN, Protocol::TEST_HOSTNAME,
      Protocol::TEST_OS_VERSION, Protocol::TEST_CURRENT_USER);
