#ifndef METRICS_HELPER_TEST_HPP
#define METRICS_HELPER_TEST_HPP
...
inline CpuSample createCpuSample(
    float total_percent = 75.0f,
    std::vector<float> per_core = {10.0f, 20.0f, 30.0f, 40.0f}) {
  CpuSample cpu;
...
inline MemSample createMemSample(
    std::uint64_t phys_total     = 8000000000ULL,
...
inline DiskSample createDiskSample(
    ...
    std::uint64_t write_bytes_per_sec = 200000ULL,
...
inline NetSample createNetSample(
    std::uint64_t rx_bytes_per_sec = 500000ULL,
...
inline MetricsSample createMetricsSample(int diskCount = 2, int ifaceCount = 2) {
  MetricsSample sample;
  sample.cpu = createCpuSample();
  sample.mem = createMemSample();
  for (int i{0}; i < diskCount; ++i)
    sample.disks.push_back(createDiskSample());
  for (int i{0}; i < ifaceCount; ++i)
    sample.interfaces.push_back(createNetSample());
  return sample;
}
