...
bool WindowsSocket::connect(const std::string &host, uint16_t port)
{
    ...
    const bool ok = ::connect(socket_, result->ai_addr,
...
SocketResult WindowsSocket::send(const uint8_t *data, size_t len)
{
    ...
    if (sent == SOCKET_ERROR)
        return { -1, translateStatus(WSAGetLastError()) };
...
{
    const int received = ::recv(socket_, reinterpret_cast<char *>(data),
                                static_cast<int>(len), 0);
...
std::string WindowsSocket::remoteAddress() const
{
    sockaddr_in addr{};
    int len = sizeof(addr);
    if (getpeername(socket_, reinterpret_cast<sockaddr *>(&addr), &len) == SOCKET_ERROR)
        return {};
...
{
    switch (err) {
    case WSAECONNREFUSED:  return SocketStatus::CONNECTION_REFUSED;
    case WSAECONNRESET:    return SocketStatus::CONNECTION_RESET;
    case WSAETIMEDOUT:     return SocketStatus::TIMED_OUT;
    case WSAEWOULDBLOCK:   return SocketStatus::WOULD_BLOCK;
    case WSAEADDRINUSE:    return SocketStatus::ADDRESS_IN_USE;
    case WSAEINVAL:        return SocketStatus::INVALID_ARGUMENT;
    case WSAENETUNREACH:   return SocketStatus::NETWORK_UNREACHABLE;
